#!/usr/bin/env python3
"""
MiniMax Token Plan MCP Client - Image Understanding
处理 MCP 协议的初始化和工具调用
"""
...
def main():
    if len(sys.argv) < 3:
        print("Usage: python3 understand.py <prompt> <image_url_or_path>", file=sys.stderr)
        print("Example: python3 understand.py \"这张图片里有什么？\" \"https://example.com/image.jpg\"", file=sys.stderr)
        print("Example: python3 understand.py \"描述这张图片\" \"/path/to/image.png\"", file=sys.stderr)
        sys.exit(1)
    ...
    try:
        ...
        # 使用 browse 工具进行图片理解
        result = client.call_tool("browse", {
            "urls": [image_url],
            "query": prompt
        })
